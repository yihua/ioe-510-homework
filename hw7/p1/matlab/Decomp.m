...
% sformat short e
n = 6 % number of x variables
m1 = 1 % number of 'complicating' equations = number of s variables
m2 = 2 % number of 'nice' equations 

rng('default');
rng(328);  % set seed
E = ceil(15.0*rand(m1,n))  % random m1-by-n matrix
%E = [0 -4 0 -1 0]
A = ceil(15.0*rand(m2,n))  % random m2-by-n matrix
%A = [1 -1 0 -1 0; 0 -4 2 2 0; 0 -9 0 6 3]

w = rand(n,1);     % generate rhs's so problem is feasible
h = ceil(E*w + 0.01*rand(m1,1))
%h = [0]
b = ceil(A*w)
%b = [1 2 18]';   

c = ceil(10.0*rand(n,1))     % generate random objective
%c = [16 7 20 10 4]';
